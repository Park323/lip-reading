...
class LinearSubsampling(torch.nn.Module):
    """Linear subsampling (to 1/4 length).
    ...
    def __init__(self, idim, odim, dropout_rate, pos_enc=None, scale_factor=0.25, mode='linear'):
        """Construct an LinearSubsampling object."""
        super(LinearSubsampling, self).__init__()
        self.downsample = torch.nn.Upsample(scale_factor=scale_factor, mode=mode)
    ...
    def forward(self, x, x_mask):
        ...
        """
        x = self.downsample(x.transpose(1,2).contiguous())
